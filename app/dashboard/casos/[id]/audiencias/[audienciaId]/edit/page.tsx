...
export default function EditAudienciaPage() {
  const { id, audienciaId } = useParams() as {
    id: string;
    audienciaId: string;
  };
  ...
  const [audiencia, setAudiencia] = useState<CasoAudiencia | null>(null);
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      if (active) setCaso(casoData);

      // 3) Load hearing
      const { data: audienciaData, error } = await getAudienciaById(audienciaId);
      if (!active) return;

      if (error || !audienciaData) {
      ...
      const { fecha: f, hora: h } = splitDateTime(audienciaData.fecha);

      setFecha(f);
      setHora(h);
      setTipo(audienciaData.tipo);
      setSala(audienciaData.sala || '');
      setEstatus(audienciaData.estatus);
      setNotas(audienciaData.notas || '');
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div className="flex items-center justify-between gap-3 pt-2">
            <Button type="button" variant="outline" asChild disabled={saving}>
              <Link href={`/dashboard/casos/${id}`}>Cancelar</Link>
            </Button>
